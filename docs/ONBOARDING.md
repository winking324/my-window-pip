...
- **拖动是手动实现的**：`isMovableByWindowBackground = false`，由 `PiPContentView` 在 `mouseDragged` 里按位移 `setFrameOrigin`。原因是系统背景拖动会吞掉 `mouseUp`，拿不到干净的单击，而单击要用来「切回源应用」。拖动会经过 `SessionStore` 的多窗口磁吸解析，只修正位置、不改尺寸，屏幕可用区域边缘和相邻浮窗的磁吸间距都为 0；候选窗口还必须在当前 Space 实际可见，不能只检查 `state.isHidden`，否则普通置顶模式会被其他 Space 的旧 frame 干扰。屏幕边缘磁吸使用 `NSScreen.visibleFrame`，以避开菜单栏和 Dock；额外内缩为 0。改动手势时注意保持「拖动后触发 `pipDidMove` 持久化」与「跨屏 scale 变化后 retune」两条链路。
